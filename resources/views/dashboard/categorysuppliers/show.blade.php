    ...
    <div class="w-full max-w-lg mt-16">
        <div class="flex p-1 w-full">
            ...
            @canany(['isAdmin', 'isMedia', 'isMarketing'])
                <div class="border-b">
                    <a href="/dashboard/categorysupliers/{{ $categorysupplier->id }}/edit" class="index-link btn-primary">
        ...
        <div class="bg-white p-5 rounded-lg shadow-lg mt-5">
            <h2 class="text-2xl font-bold text-teal-700 mb-4">{{ $categorysupplier->name }}</h2>
            <div class="mb-4">
                <strong class="text-teal-700">Kode:</strong>
                <p class="text-slate-700">{{ $categorysupplier->code }}</p>
            </div>
            <div class="mb-4">
                <strong class="text-teal-700">Deskripsi:</strong>
                <p class="text-slate-700">{{ $categorysupplier->description }}</p>
            </div>
            @canany(['isAdmin', 'isMedia', 'isMarketing'])
                <form action="/dashboard/categorysuppliers/{{ $categorysupplier->id }}" method="post" class="mt-4">
                    @method('delete')
                    @csrf
                    <button class="btn-danger text-white w-full py-2 rounded bg-red-600 hover:bg-red-700 flex items-center justify-center" onclick="return confirm('Apakah anda yakin ingin menghapus {{ $categorysupplier->name }} dengan kode {{ $categorysupplier->code }} ?')">
