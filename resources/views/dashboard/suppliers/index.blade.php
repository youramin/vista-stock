    ...
    <div class="mt-16 z-0 mb-8">
        <div class="flex p-1 w-full">
            ...
            @canany(['isAdmin', 'isMedia', 'isMarketing'])
                <div class="border-b">
                    <a href="/dashboard/suppliers/create" class="index-link btn-primary">
                        ...
                        <span class="mx-1 hidden sm:flex">Tambah Barang</span>
                    </a>
                </div>
            @endcanany
        </div>
        <div class="">
            <form action="/dashboard/suppliers">
            ...
            <table class="table-auto w-full">
                <thead>
                    <tr class="bg-teal-100 h-10">
                        ...
                        <th class="text-teal-700 border text-sm text-center w-24">Nama Supplier</th>
                        <th class="text-teal-700 border text-sm text-center w-30">Kategori Supplier</th>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="text-teal-700 border text-sm text-center">{{ $supplier->code }}</td>
                        <td class="text-teal-700 border text-sm text-center">{{ $supplier->name }}</td>
                        <td class="text-teal-700 border text-sm text-center">{{ $supplier->categorysupplier->name}}</td>
                        <td class="text-teal-700 border text-sm text-center">{{ $supplier->description }}</td>
                                ...
                                @canany(['isAdmin', 'isMedia', 'isMarketing'])
                                    ...
                                    <form action="/dashboard/suppliers/{{ $supplier->id }}"
                                        ...
                                        <button
                                            ...
                                            onclick="return confirm('Apakah anda yakin ingin menghapus {{ $supplier->name }} dengan kode {{ $supplier->code }} ?')">
                                            <svg class="fill-current w-[18px] 2xl:w-5" clip-rule="evenodd"
                                                fill-rule="evenodd" stroke-linejoin="round" stroke-miterlimit="2"
                                                viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                                <path
